module.exports = function(grunt) {
  ...
  var scriptPaths = require('./src/pixetor-script-list.js').scripts;
  var pixetorScripts = prefixPaths(scriptPaths, "src/").filter(function (path) {
    return path.indexOf('devtools') === -1;
  });

  // get the list of styles paths to include
  var stylePaths = require('./src/pixetor-style-list.js').styles;
  var pixetorStyles = prefixPaths(stylePaths, "src/");
  ...
  grunt.initConfig({
    ...
    concat : {
      js : {
        ...
        src : pixetorScripts,
        dest : 'dest/prod/js/pixetor-packaged' + version + '.js'
      },
      css : {
        src : pixetorStyles,
        dest : 'dest/tmp/css/pixetor-style-packaged' + version + '.css'
      }
    },
    ...
    uglify : {
      ...
      js : {
        files : {
          'dest/tmp/js/pixetor-packaged-min.js' : ['dest/prod/js/pixetor-packaged' + version + '.js']
        }
      }
    },
    ...
    replace: {
      // main-partial.html is used when embedded in pixetor
      mainPartial: {
        options: {
          patterns: [{
              match: /^(.|[\r\n])*<!--body-main-start-->/,
        ...
        files: [
          ...
          {src: ['dest/tmp/index.html'], dest: 'dest/prod/pixetorapp-partials/main-partial.html'}
        ]
      },
      ...
      css: {
        ...
        files: [{
          src: ['dest/tmp/css/pixetor-style-packaged' + version + '.css'],
          dest: 'dest/prod/css/pixetor-style-packaged' + version + '.css'
        }]
      }
    },
    ...
    copy: {
      prod: {
        files: [
          // dest/js/pixetor-packaged-min.js should have been created by the uglify task
          {src: ['dest/tmp/js/pixetor-packaged-min.js'], dest: 'dest/prod/js/pixetor-packaged-min' + version + '.js'},
      ...
      dev: {
        files: [
          ...
          {src: ['src/pixetor-script-list.js'], dest: 'dest/dev/pixetor-script-list.js'},
          {src: ['src/pixetor-style-list.js'], dest: 'dest/dev/pixetor-style-list.js'},
