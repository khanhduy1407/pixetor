(function () {
  var ns = $.namespace('pxtr.rendering.frame');
  ...
  pxtr.utils.inherit(pxtr.rendering.frame.FrameRenderer, pxtr.rendering.AbstractRenderer);
  ...
  ns.FrameRenderer.prototype.clear = function () {
    pxtr.utils.CanvasUtils.clear(this.canvas);
    pxtr.utils.CanvasUtils.clear(this.displayCanvas);
  };
  ...
  ns.FrameRenderer.prototype.setZoom = function (zoom) {
    ...
    zoom = pxtr.utils.Math.minmax(zoom, minimumZoom, maximumZoom);
  ...
  ns.FrameRenderer.prototype.setOffset = function (x, y) {
    var width = pxtr.app.pixetorController.getWidth();
    var height = pxtr.app.pixetorController.getHeight();
    var maxX = width - (this.displayWidth / this.zoom);
    x = pxtr.utils.Math.minmax(x, 0, maxX);
    var maxY = height - (this.displayHeight / this.zoom);
    y = pxtr.utils.Math.minmax(y, 0, maxY);
  ...
  ns.FrameRenderer.prototype.getGridColor = function () {
    return pxtr.UserSettings.get(pxtr.UserSettings.GRID_COLOR);
  };
  ...
  ns.FrameRenderer.prototype.createDisplayCanvas_ = function () {
    ...
    this.displayCanvas = pxtr.utils.CanvasUtils.createCanvas(width, height, this.classList);
    pxtr.utils.CanvasUtils.disableImageSmoothing(this.displayCanvas);
    this.container.appendChild(this.displayCanvas);
  };

  ns.FrameRenderer.prototype.onUserSettingsChange_ = function (evt, settingName, settingValue) {
    var settings = pxtr.UserSettings;
  ...
  ns.FrameRenderer.prototype.getUserGridWidth_ = function () {
    ...
    var width = pxtr.UserSettings.get(pxtr.UserSettings.GRID_WIDTH);
    return gridEnabled ? width : 0;
  };

  ns.FrameRenderer.prototype.getUserGridSpacing_ = function () {
    var gridEnabled = pxtr.UserSettings.get(pxtr.UserSettings.GRID_ENABLED);
    var spacing = pxtr.UserSettings.get(pxtr.UserSettings.GRID_SPACING);
  ...
  ns.FrameRenderer.prototype.renderFrame_ = function (frame) {
    if (!this.canvas || frame.getWidth() != this.canvas.width || frame.getHeight() != this.canvas.height) {
      this.canvas = pxtr.utils.CanvasUtils.createCanvas(frame.getWidth(), frame.getHeight());
    }
    ...
    // Draw in canvas
    pxtr.utils.FrameUtils.drawToCanvas(frame, this.canvas);
    ...
    // on some chromebooks (cf https://github.com/pixetorapp/pixetor/issues/651)
    if (isZoomedOut) {
      // Draw background
    ...
    if (pxtr.UserSettings.get('SEAMLESS_MODE')) {
  ...
  ns.FrameRenderer.prototype.drawTiledFrames_ = function (context, image, w, h, z) {
    var opacity = pxtr.UserSettings.get('SEAMLESS_OPACITY');
    opacity = pxtr.utils.Math.minmax(opacity, 0, 1);
