(function () {
  ...
  var DB_NAME = 'PixetorSessionsDatabase';
  var DB_VERSION = 1;
   ...
   * to pixetor snapshots continuously saved while during the usage of
   * Pixetor.
   */
  ...
  ns.BackupDatabase.prototype.init = function () {
    ...
    return _requestPromise(request).then(function (event) {
      ...
      console.log('Could not initialize the pixetor backup database');
    });
  };
  ...
  ns.BackupDatabase.prototype.getSnapshotsBySessionId = function (sessionId) {
    ...
    // Loop on all the saved snapshots for the provided pixetor id
    ...
    index.openCursor(keyRange, 'prev').onsuccess = function(event) {
      ...
      if (cursor) {
        ...
        // Consumed all pixetor snapshots
