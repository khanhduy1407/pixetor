(function () {
  ...
  ns.CanvasUtils = {
    ...
    createFromImageData : function (imageData) {
      var canvas = pxtr.utils.CanvasUtils.createCanvas(imageData.width, imageData.height);
    ...
    createFromImage : function (image) {
      var canvas = pxtr.utils.CanvasUtils.createCanvas(image.width, image.height);
      var context = canvas.getContext('2d');
    ...
    createFramesFromImage : function (image, offsetX, offsetY, width, height, useHorizonalStrips, ignoreEmptyFrames) {
      ...
      var x = offsetX;
      var y = offsetY;
      var blankData = pxtr.utils.CanvasUtils.createCanvas(width, height).toDataURL();
    ...
    disableImageSmoothing : function (canvas) {
      pxtr.utils.CanvasUtils.setImageSmoothing(canvas, false);
    },

    enableImageSmoothing : function (canvas) {
      pxtr.utils.CanvasUtils.setImageSmoothing(canvas, true);
    },
    ...
    clone : function (canvas) {
      var clone = pxtr.utils.CanvasUtils.createCanvas(canvas.width, canvas.height);
