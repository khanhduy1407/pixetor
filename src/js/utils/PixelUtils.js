(function () {
  ...
  ns.PixelUtils = {
     ...
     *
     * @param frame pxtr.model.Frame The frame target in which we want to paintbucket
    ...
    getSimilarConnectedPixelsFromFrame: function(frame, col, row) {
      ...
      var visited = {};
      return pxtr.PixelUtils.visitConnectedPixels(startPixel, frame, function (pixel) {
    ...
    /**
     * Shortcut to reduce the output of pxtr.PixelUtils.resizePixel for several pixels
    ...
    resizePixels : function (pixels, size) {
      return pixels.reduce(function (p, pixel) {
        return p.concat(pxtr.PixelUtils.resizePixel(pixel.col, pixel.row, size));
      }, []);
    },
    ...
    paintSimilarConnectedPixelsFromFrame: function(frame, col, row, replacementColor) {
      ...
      if (typeof replacementColor == 'string') {
        replacementColor = pxtr.utils.colorToInt(replacementColor);
      }
      ...
      var paintedPixels = pxtr.PixelUtils.visitConnectedPixels(startPixel, frame, function (pixel) {
     ...
     *
     * TODO : The visitor is also responsible for making sure a pixel is never
    ...
    getLinePixels : function (x0, x1, y0, y1) {
      ...
      x1 = pxtr.utils.normalize(x1, 0);
      y1 = pxtr.utils.normalize(y1, 0);
    ...
    getUniformLinePixels : function (x0, x1, y0, y1) {
      ...
      var maxDistance = pxtr.utils.Math.distance(x0, x1, y0, y1);
      ...
      while (true) {
        ...
        pixels.push({'col': x, 'row': y});
        if (pxtr.utils.Math.distance(x0, x, y0, y) >= maxDistance) {
          break;
        }
