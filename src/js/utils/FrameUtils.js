(function () {
  ...
  ns.FrameUtils = {
    ...
    toImage : function (frame, zoom, opacity) {
      ...
      var canvasRenderer = new pxtr.rendering.CanvasRenderer(frame, zoom);
    ...
    drawToCanvas : function (frame, canvas, transparentColor, globalAlpha) {
      ...
      if (frame instanceof pxtr.model.frame.RenderedFrame) {
        ...
        var constantTransparentColorInt = pxtr.utils.colorToInt(Constants.TRANSPARENT_COLOR);
        var transparentColorInt = pxtr.utils.colorToInt(transparentColor);
        ...
        if (!offCanvasPool[offCanvasKey]) {
          offCanvas = offCanvasPool[offCanvasKey] = pxtr.utils.CanvasUtils.createCanvas(w, h);
    ...
    merge : function (frames) {
      ...
      if (frames.length) {
        ...
        for (var i = 1 ; i < frames.length ; i++) {
          pxtr.utils.FrameUtils.mergeFrames_(merged, frames[i]);
        }
      }
      return merged;
    },
    ...
    mergeFrames_ : function (frameA, frameB) {
      var transparentColorInt = pxtr.utils.colorToInt(Constants.TRANSPARENT_COLOR);
    ...
    addImageToFrame: function (frame, image, x, y) {
      var imageFrame = pxtr.utils.FrameUtils.createFromImage(image);
      ...
      imageFrame.forEachPixel(function (color, frameX, frameY) {
        if (color != pxtr.utils.colorToInt(Constants.TRANSPARENT_COLOR)) {
    ...
    resize : function (frame, targetWidth, targetHeight, smoothing) {
      ...
      var resizedImage = pxtr.utils.ImageResizer.resize(image, targetWidth, targetHeight, smoothing);
    ...
    createFromCanvas : function (canvas, x, y, w, h, preserveOpacity) {
      ...
      return pxtr.utils.FrameUtils.createFromImageData_(imgData, w, h, preserveOpacity);
    },
    ...
    /*
     * Create a pxtr.model.Frame from an Image object. By default transparent
     ...
     * @return {pxtr.model.Frame} corresponding frame
     */
    createFromImage : function (image, preserveOpacity) {
      var w = image.width;
      var h = image.height;
      var canvas = pxtr.utils.CanvasUtils.createCanvas(w, h);
      var context = canvas.getContext('2d');
    ...
    createFromImageData_ : function (imageData, width, height, preserveOpacity) {
      ...
      if (!preserveOpacity) {
        pxtr.utils.FrameUtils.removeTransparency(frame);
      }
    ...
    createFramesFromSpritesheet : function (image, frameCount) {
      ...
      var chunkFrames = pxtr.utils.FrameUtils.createFramesFromChunk(image, layout);
    ...
    createFramesFromChunk : function (image, layout) {
      ...
      var canvas = pxtr.utils.CanvasUtils.createCanvas(frameWidth, frameHeight);
      var context = canvas.getContext('2d');
      ...
      for (var i = 0 ; i < layout.length ; i++) {
        ...
        for (var j = 0 ; j < row.length ; j++) {
          ...
          var frame = pxtr.utils.FrameUtils.createFromCanvas(canvas, 0, 0, frameWidth, frameHeight);
          chunkFrames.push({
            index : layout[i][j],
            frame : frame
          });
