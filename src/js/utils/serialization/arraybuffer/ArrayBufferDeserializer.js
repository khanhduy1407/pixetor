(function () {
  ...
  ns.ArrayBufferDeserializer = {
    deserialize : function (data, callback) {
      ...
      var descriptor = new pxtr.model.pixetor.Descriptor(descriptorName, descriptorDescription);
      var pixetor = new pxtr.model.Pixetor(width, height, fps, descriptor);
      pixetor.hiddenFrames = hiddenFrames;
      var loadedLayers = 0;

      var loadLayerImage = function(layer, cb) {
        var image = new Image();
        image.onload = function() {
          var frames = pxtr.utils.FrameUtils.createFramesFromSpritesheet(this, layer.frameCount);
      ...
      for (i = 0; i < layerCount; i++) {
        layer = layers[i];
        var nlayer = new pxtr.model.Layer(layer.name);
        layer.model = nlayer;
        nlayer.setOpacity(layer.opacity);
        pixetor.addLayer(nlayer);
