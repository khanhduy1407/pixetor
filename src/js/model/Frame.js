(function () {
  ...
  ns.Frame = function (width, height) {
    if (width && height) {
      ...
      throw 'Bad arguments in pxtr.model.Frame constructor : ' + width + ', ' + height;
    }
  };
  ...
  ns.Frame.fromPixelGrid = function (pixels, width, height) {
    if (pixels.length) {
      ...
      if (pixels[0].length) {
        ...
        for (var y = 0; y < h; y++) {
          for (var x = 0; x < w; x++) {
            if (typeof pixels[x][y] == 'string') {
              buffer[y * w + x] = pxtr.utils.colorToInt(pixels[x][y]);
            } else {
        ...
        throw 'Bad arguments in pxtr.model.frame.fromPixelGrid, missing width and height';
      }

      var frame = new pxtr.model.Frame(w, h);
      frame.setPixels(buffer);
      return frame;
    } else {
      throw 'Bad arguments in pxtr.model.Frame.fromPixelGrid';
    }
  };
  ...
  ns.Frame.prototype.setPixel = function (x, y, color) {
    if (this.containsPixel(x, y)) {
      ...
      color = pxtr.utils.colorToInt(color);

      if (p !== color) {
        this.pixels[index] = color || pxtr.utils.colorToInt(Constants.TRANSPARENT_COLOR);
        this.version++;
      }
    }
  };
