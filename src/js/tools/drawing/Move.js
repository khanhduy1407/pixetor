/**
 * @provide pxtr.tools.drawing.Move
 *
 * @require pxtr.utils
 */
(function() {
  ...
  ns.Move = function() {
    ...
    this.shortcut = pxtr.service.keyboard.Shortcuts.TOOL.MOVE;
  ...
  pxtr.utils.inherit(ns.Move, ns.BaseTool);
  ...
  ns.Move.prototype.releaseToolAt = function(col, row, frame, overlay, event) {
    ...
    var ctrlKey = pxtr.utils.UserAgent.isMac ?  event.metaKey : event.ctrlKey;
    pxtr.tools.ToolsHelper.getTargetFrames(ctrlKey, event.shiftKey).forEach(function (f) {
  ...
  ns.Move.prototype.replay = function(frame, replayData) {
    ...
    pxtr.tools.ToolsHelper.getTargetFrames(event.ctrlKey, event.shiftKey).forEach(function (frame) {
