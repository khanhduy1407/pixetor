/**
 * @provide pxtr.tools.drawing.Stroke
 *
 * @require pxtr.utils
 */
(function() {
  ...
  ns.Stroke = function() {
    ...
    this.shortcut = pxtr.service.keyboard.Shortcuts.TOOL.STROKE;
  ...
  pxtr.utils.inherit(ns.Stroke, ns.BaseTool);
  ...
  ns.Stroke.prototype.draw_ = function (col, row, color, targetFrame, penSize, isStraight) {
    var linePixels;
    if (isStraight) {
      linePixels = pxtr.PixelUtils.getUniformLinePixels(this.startCol, col, this.startRow, row);
    } else {
      linePixels = pxtr.PixelUtils.getLinePixels(col, this.startCol, row, this.startRow);
    }

    //draw the square ends of the line
    pxtr.PixelUtils.resizePixel(linePixels[0].col, linePixels[0].row, penSize)
      .forEach(function (point) {targetFrame.setPixel(point[0], point[1], color);});
    pxtr.PixelUtils.resizePixel(linePixels[linePixels.length - 1].col, linePixels[linePixels.length - 1].row, penSize)
      .forEach(function (point) {targetFrame.setPixel(point[0], point[1],color);});
