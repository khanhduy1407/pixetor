/**
 * @provide pxtr.tools.drawing.selection.LassoSelect
 *
 * @require pxtr.utils
 */
(function() {
  ...
  ns.LassoSelect = function() {
    ...
    this.shortcut = pxtr.service.keyboard.Shortcuts.TOOL.LASSO_SELECT;
  };

  pxtr.utils.inherit(ns.LassoSelect, ns.AbstractDragSelect);
  ...
  ns.LassoSelect.prototype.onDragSelect_ = function (col, row, frame, overlay) {
    ...
    var selection = new pxtr.selection.ShapeSelection(this.getLassoPixels_());
  ...
  ns.LassoSelect.prototype.onDragSelectEnd_ = function (col, row, frame, overlay) {
    ...
    var selection = new pxtr.selection.LassoSelection(this.getLassoPixels_(), frame);
    this.setSelection_(selection, overlay);
  ...
  ns.LassoSelect.prototype.getLassoPixels_ = function () {
    var line = pxtr.PixelUtils.getLinePixels(this.previousCol, this.startCol, this.previousRow, this.startRow);
  ...
  ns.LassoSelect.prototype.addPixel_ = function (col, row, frame) {
    ...
    col = pxtr.utils.Math.minmax(col, 0, frame.getWidth() - 1);
    row = pxtr.utils.Math.minmax(row, 0, frame.getHeight() - 1);
