(function () {
  ...
  ns.SelectionManager = function (pixetorController) {
  ...
  ns.SelectionManager.prototype.init = function () {
    ...
    pxtr.app.shortcutService.registerShortcut(shortcuts.SELECTION.DELETE, this.onDeleteShortcut_.bind(this));
    pxtr.app.shortcutService.registerShortcut(shortcuts.SELECTION.COMMIT, this.commit.bind(this));
  ...
  ns.SelectionManager.prototype.onToolSelected_ = function(evt, tool) {
    var isSelectionTool = tool instanceof pxtr.tools.drawing.selection.BaseSelect;
    if (!isSelectionTool) {
  ...
  ns.SelectionManager.prototype.copy = function(event, domEvent) {
    if (this.currentSelection && this.pixetorController.getCurrentFrame()) {
      this.currentSelection.fillSelectionFromFrame(this.pixetorController.getCurrentFrame());
      if (domEvent) {
  ...
  ns.SelectionManager.prototype.paste = function(event, domEvent) {
    ...
    try {
      ...
      // Allow Pixetor to fallback on local currentSelection pasting.
    }
  ...
  ns.SelectionManager.prototype.pasteImage_ = function(clipboardItem) {
    ...
    pxtr.utils.FileUtils.readImageFile(blob, function (image) {
      pxtr.app.fileDropperService.dropPosition_ = {x: 0, y: 0};
      pxtr.app.fileDropperService.onImageLoaded_(image, blob);
    }.bind(this));
  };
  ...
  ns.SelectionManager.prototype.commit = function() {
    var tool = pxtr.app.drawingController.currentToolBehavior;
    var isSelectionTool = tool instanceof pxtr.tools.drawing.selection.BaseSelect;
    if (isSelectionTool) {
