(function () {
  ...
  ns.PaletteImageReader = function (file, onSuccess, onError) {
    ...
    this.colorSorter_ = new pxtr.service.color.ColorSorter();
  };

  ns.PaletteImageReader.prototype.read = function () {
    pxtr.utils.FileUtils.readImageFile(this.file, this.onImageLoaded_.bind(this));
  };

  ns.PaletteImageReader.prototype.onImageLoaded_ = function (image) {
    var imageProcessor = new pxtr.worker.imageprocessor.ImageProcessor(image,
  ...
  ns.PaletteImageReader.prototype.onWorkerSuccess_ = function (event) {
    ...
    if (colors.length > Constants.MAX_PALETTE_COLORS) {
      ...
      var palette = new pxtr.model.Palette(uuid, this.file.name + ' palette', sortedColors);
