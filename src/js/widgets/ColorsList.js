(function () {
  ...
  ns.ColorsList = function (container) {
    ...
    this.palette = new pxtr.model.Palette('tmp', 'tmp', []);
    ...
    pxtr.utils.Event.addEventListener(this.colorsList, 'click', this.onColorContainerClick_, this);

    var colorPickerContainer = container.querySelector('.color-picker-container');
    this.hslRgbColorPicker = new pxtr.widgets.HslRgbColorPicker(colorPickerContainer, this.onColorUpdated_.bind(this));
  ...
  ns.ColorsList.prototype.refresh_ = function () {
    var html = '';
    var tpl = pxtr.utils.Template.get('create-palette-color-template');
    var colors = this.palette.getColors();

    colors.forEach(function (color, index) {
      var isSelected = (index === this.selectedIndex);

      html += pxtr.utils.Template.replace(tpl, {
