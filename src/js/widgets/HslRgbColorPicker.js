(function () {
  ...
  ns.HslRgbColorPicker.prototype.init = function () {
    var isFirefox = pxtr.utils.UserAgent.isFirefox;
    var isChrome = pxtr.utils.UserAgent.isChrome;

    var changeEvent = (isChrome || isFirefox) ? 'input' : 'change';
    pxtr.utils.Event.addEventListener(this.container, changeEvent, this.onPickerChange_, this);
    pxtr.utils.Event.addEventListener(this.container, 'keydown', this.onPickerChange_, this);

    // Cannot use pxtr.utils.Event with useCapture for now ...
  ...
  ns.HslRgbColorPicker.prototype.getIncrement_ = function (evt) {
    var increment = 0;
    var key = pxtr.service.keyboard.KeycodeTranslator.toChar(evt.keyCode);
    if (key === 'up') {
      increment = 1;
    } else if (key === 'down') {
      increment = -1;
  ...
  ns.HslRgbColorPicker.prototype.toTinyColor_ = function (color) {
    ...
    if (isTinyColor) {
      ...
      return window.tinycolor(pxtr.utils.copy(color));
    }
  };
  ...
  ns.HslRgbColorPicker.prototype.getSliderBackgroundColors_ = function (model, dimension) {
    var color = this.getColor_(model);
    var start = pxtr.utils.copy(color);
    var end = pxtr.utils.copy(color);
