(function () {
  ...
  ns.PalettesListController.prototype.init = function () {
    this.paletteColorTemplate_ = pxtr.utils.Template.get('palette-color-template');
    ...
    pxtr.app.shortcutService.registerShortcut(shortcuts.COLOR.PREVIOUS_COLOR, this.selectPreviousColor_.bind(this));
    pxtr.app.shortcutService.registerShortcut(shortcuts.COLOR.NEXT_COLOR, this.selectNextColor_.bind(this));
    pxtr.app.shortcutService.registerShortcut(shortcuts.COLOR.SELECT_COLOR, this.selectColorForKey_.bind(this));
  ...
  ns.PalettesListController.prototype.fillPaletteList = function () {
    ...
    var html = palettes.map(function (palette) {
      return pxtr.utils.Template.replace('<option value="{{id}}">{{name}}</option>', palette);
    }).join('');
  ...
  ns.PalettesListController.prototype.fillColorListContainer = function () {
    ...
    if (colors.length > 0) {
      var html = colors.filter(function (color) {
        ...
        return pxtr.utils.Template.replace(this.paletteColorTemplate_, {
          color : color,
          index : index + 1,
      ...
      this.colorListContainer_.innerHTML = pxtr.utils.Template.get('palettes-list-no-colors-partial');
    }
  ...
  ns.PalettesListController.prototype.selectPalette = function (paletteId) {
    pxtr.UserSettings.set(pxtr.UserSettings.SELECTED_PALETTE, paletteId);
  };
  ...
  ns.PalettesListController.prototype.onUserSettingsChange_ = function (evt, name, value) {
    if (name == pxtr.UserSettings.SELECTED_PALETTE) {
      this.updateFromUserSettings();
    }
  };
  ...
  ns.PalettesListController.prototype.highlightSelectedColors = function () {
    ...
    var colorContainer = this.getColorContainer_(pxtr.app.selectedColorsService.getSecondaryColor());
    if (colorContainer) {
      colorContainer.classList.remove(PRIMARY_COLOR_CLASSNAME);
      colorContainer.classList.add(SECONDARY_COLOR_CLASSNAME);
    }

    colorContainer = this.getColorContainer_(pxtr.app.selectedColorsService.getPrimaryColor());
    if (colorContainer) {
