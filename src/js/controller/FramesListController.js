(function () {
  ...
  ns.FramesListController = function (pixetorController, container) {
    ...
    this.cachedFrameProcessor = new pxtr.model.frame.CachedFrameProcessor();
  ...
  ns.FramesListController.prototype.init = function() {
    ...
    $.subscribe(Events.PIXETOR_RESET, this.flagForRedraw_.bind(this, true));
    $.subscribe(Events.USER_SETTINGS_CHANGED, this.flagForRedraw_.bind(this));

    $.subscribe(Events.PIXETOR_RESET, this.refreshZoom_.bind(this));
  ...
  ns.FramesListController.prototype.onContainerClick_ = function (event) {
    var target = pxtr.utils.Dom.getParentWithData(event.target, 'tileAction');
    if (!target) {
      return;
    }
    ...
    if (action === ACTION.CLONE) {
      this.pixetorController.duplicateFrameAt(index);
  ...
  ns.FramesListController.prototype.updatePreviews_ = function () {
    ...
    for (i = 0, length = this.tiles.length; i < length; i++) {
      ...
      // Update visibility
      if (this.pixetorController.hasVisibleFrameAt(i)) {
        this.tiles[i].querySelector('.tile-count').classList.add('toggled');
      }

      // Check if any tile is updated
      var hash = this.pixetorController.getCurrentLayer().getFrameAt(i).getHash();
      if (this.tiles[i].getAttribute('data-tile-hash') !== hash) {
        if (this.tiles[i].querySelector('canvas')) {
          this.tiles[i].querySelector('.canvas-container').replaceChild(
            this.getCanvasForFrame(this.pixetorController.getCurrentLayer().getFrameAt(i)),
    ...
    var display = (this.pixetorController.getFrameCount() > 1) ? 'block' : 'none';
    for (i = 0, length = buttons.length; i < length; i++) {
      buttons[i].style.display = display;
    }

    // Add selected class
    this.tiles[this.pixetorController.getCurrentFrameIndex()].classList.add('selected');
  };
  ...
  ns.FramesListController.prototype.onUpdate_ = function (event, ui) {
    ...
    this.pixetorController.moveFrame(originFrameId, targetInsertionId);
    this.pixetorController.setCurrentFrameIndex(targetInsertionId);
  ...
  ns.FramesListController.prototype.createPreviewTile_ = function(tileNumber) {
    var currentFrame = this.pixetorController.getCurrentLayer().getFrameAt(tileNumber);
    ...
    if (this.pixetorController.getCurrentFrame() == currentFrame) {
      previewTileRoot.classList.add('selected');
    }

    var canvasContainer = document.createElement('div');
    canvasContainer.classList.add('canvas-container', pxtr.UserSettings.get(pxtr.UserSettings.CANVAS_BACKGROUND));

    var height = this.zoom * this.pixetorController.getCurrentFrame().getHeight();
    var horizontalMargin = (Constants.PREVIEW_FILM_SIZE - height) / 2;
    canvasContainer.style.marginTop = horizontalMargin + 'px';

    var width = this.zoom * this.pixetorController.getCurrentFrame().getWidth();
  ...
  ns.FramesListController.prototype.frameToPreviewCanvas_ = function (frame) {
    var canvasRenderer = new pxtr.rendering.CanvasRenderer(frame, this.zoom);
  ...
  ns.FramesListController.prototype.clonePreviewCanvas_ = function (canvas) {
    var clone = pxtr.utils.CanvasUtils.clone(canvas);
    clone.classList.add('tile-view', 'canvas');
    return clone;
  };

  /**
   * Calculate the preview zoom depending on the pixetor size
   */
