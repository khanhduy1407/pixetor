(function () {
  ...
  ns.ImageImport = function (pixetorController, importController, container) {
    this.superclass.constructor.apply(this, arguments);
    this.importedImage_ = null;
    this.file_ = null;
  };

  pxtr.utils.inherit(ns.ImageImport, ns.AbstractImportStep);
  ...
  ns.ImageImport.prototype.init = function (file) {
    ...
    pxtr.utils.FileUtils.readImageFile(this.file_, this.onImageLoaded_.bind(this));
  ...
  ns.ImageImport.prototype.onNextClick = function () {
    ...
    this.createPixetorFromImage().then(function (pixetor) {
  ...
  ns.ImageImport.prototype.createPixetorFromImage = function () {
    var name = this.extractFileNameFromPath_(this.file_.name);
    // Remove extension from filename.
    name = name.replace(/\.[a-zA-Z]+$/, '');

    var deferred = Q.defer();
    pxtr.app.importService.newPixetorFromImage(
      this.importedImage_,
      {
  ...
  ns.ImageImport.prototype.drawFrameGrid_ = function (frameX, frameY, frameW, frameH) {
    ...
    if (!canvas) {
      ...
      canvas = pxtr.utils.CanvasUtils.createCanvas(
        previewWidth + 1,
        previewHeight + 1);
