(function () {
  ...
  ns.CreatePaletteController = function (pixetorController) {
    this.paletteService = pxtr.app.paletteService;
    this.paletteImportService = pxtr.app.paletteImportService;
  };

  pxtr.utils.inherit(ns.CreatePaletteController, ns.AbstractDialogController);
  ...
  ns.CreatePaletteController.prototype.init = function (paletteId) {
    ...
    this.colorsListWidget = new pxtr.widgets.ColorsList(colorsListContainer);
    ...
    if (paletteId && !isCurrentColorsPalette) {
      ...
      palette = pxtr.model.Palette.fromObject(paletteObject);
    } else {
      ...
      if (isCurrentColorsPalette) {
        palette = new pxtr.model.Palette(uuid, 'Current colors clone', this.getCurrentColors_());
      } else {
        palette = new pxtr.model.Palette(uuid, 'New palette', []);
      }
    }
  ...
  ns.CreatePaletteController.prototype.setPalette_ = function (palette) {
    ...
    this.nameInput.value = pxtr.utils.unescapeHtml(palette.name);
  ...
  ns.CreatePaletteController.prototype.saveAndSelectPalette_ = function () {
    ...
    pxtr.UserSettings.set(pxtr.UserSettings.SELECTED_PALETTE, this.palette.id);
    this.closeDialog();
  };
  ...
  ns.CreatePaletteController.prototype.deletePalette_ = function () {
    if (window.confirm('Are you sure you want to delete palette ' + this.palette.name)) {
      ...
      pxtr.UserSettings.set(pxtr.UserSettings.SELECTED_PALETTE, Constants.CURRENT_COLORS_PALETTE_ID);
      this.closeDialog();
    }
  };

  ns.CreatePaletteController.prototype.onDownloadButtonClick_ = function () {
    var paletteWriter = new pxtr.service.palette.PaletteGplWriter(this.palette);
    var paletteAsString = paletteWriter.write();

    pxtr.utils.BlobUtils.stringToBlob(paletteAsString, function(blob) {
      pxtr.utils.FileUtils.downloadAsFile(blob, this.palette.name + '.gpl');
  ...
  ns.CreatePaletteController.prototype.onNameInputChange_ = function (evt) {
    this.palette.name = pxtr.utils.escapeHtml(this.nameInput.value);
  };
})();
