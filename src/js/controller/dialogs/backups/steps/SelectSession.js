(function () {
  ...
  ns.SelectSession = function (pixetorController, backupsController, container) {
  ...
  ns.SelectSession.prototype.getMarkupForSessions_ = function (sessions) {
    if (sessions.length === 0) {
      return pxtr.utils.Template.get('session-list-empty');
    }

    var sessionItemTemplate = pxtr.utils.Template.get('session-list-item');
    return sessions.reduce(function (previous, session) {
      if (session.id === pxtr.app.sessionId) {
      ...
      var view = {
        ...
        date: pxtr.utils.DateUtils.format(session.endDate, 'the {{Y}}/{{M}}/{{D}} at {{H}}:{{m}}'),
  ...
  ns.SelectSession.prototype.onContainerClick_ = function (evt) {
    ...
    if (action == 'view') {
      ...
      if (window.confirm('Are you sure you want to delete this session?')) {
        ...
        Q.all([
          pxtr.app.backupService.deleteSession(sessionId),
