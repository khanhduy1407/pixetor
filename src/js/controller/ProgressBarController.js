(function () {
  ...
  ns.ProgressBarController = function () {
    this.template = pxtr.utils.Template.get('progress-bar-template');
  ...
  ns.ProgressBarController.prototype.onTimerExpired_ = function (progressInfo) {
    var progressBarHtml = pxtr.utils.Template.replace(this.template, {
      name : progressInfo.name,
      status : 0
    });

    var progressBarEl = pxtr.utils.Template.createFromHTML(progressBarHtml);
