(function () {
  ...
  ns.DrawingController = function (pixetorController, container) {
    /**
     * @public
     */
     ...
     * @public
     */
    this.overlayFrame = pxtr.model.Frame.createEmptyFromFrame(pixetorController.getCurrentFrame());
    ...
    this.overlayRenderer = new pxtr.rendering.frame.CachedFrameRenderer(this.container, cfg, ['canvas-overlay']);
    this.onionSkinRenderer = pxtr.rendering.OnionSkinRenderer.createInContainer(this.container, cfg, pixetorController);
    this.layersRenderer = new pxtr.rendering.layer.LayersRenderer(this.container, cfg, pixetorController);
    cfg.supportGridRendering = true;
    this.renderer = new pxtr.rendering.frame.CachedFrameRenderer(this.container, cfg, ['drawing-canvas']);

    this.compositeRenderer = new pxtr.rendering.CompositeRenderer();
  ...
  ns.DrawingController.prototype.init = function () {
    ...
    pxtr.app.shortcutService.registerShortcut(shortcuts.MISC.RESET_ZOOM, this.resetZoom_.bind(this));
    pxtr.app.shortcutService.registerShortcut(shortcuts.MISC.INCREASE_ZOOM, this.updateZoom_.bind(this, 1));
    pxtr.app.shortcutService.registerShortcut(shortcuts.MISC.DECREASE_ZOOM, this.updateZoom_.bind(this, -1));
    pxtr.app.shortcutService.registerShortcut(shortcuts.MISC.OFFSET_UP, this.updateOffset_.bind(this, 'up'));
    pxtr.app.shortcutService.registerShortcut(shortcuts.MISC.OFFSET_RIGHT, this.updateOffset_.bind(this, 'right'));
    pxtr.app.shortcutService.registerShortcut(shortcuts.MISC.OFFSET_DOWN, this.updateOffset_.bind(this, 'down'));
    pxtr.app.shortcutService.registerShortcut(shortcuts.MISC.OFFSET_LEFT, this.updateOffset_.bind(this, 'left'));
  ...
  ns.DrawingController.prototype.initMouseBehavior = function() {
    ...
    if (pxtr.utils.UserAgent.isChrome || pxtr.utils.UserAgent.isIE11) {
  ...
  ns.DrawingController.prototype.onUserSettingsChange_ = function (evt, settingsName, settingsValue) {
    if (settingsName == pxtr.UserSettings.SHOW_GRID) {
      console.warn('DrawingController:onUserSettingsChange_ not implemented !');
    } else if (settingsName == pxtr.UserSettings.ONION_SKIN || settingsName == pxtr.UserSettings.LAYER_PREVIEW) {
  ...
  ns.DrawingController.prototype.moveTool_ = function (x, y, event) {
    ...
    if (this.isClicked) {
      if (pxtr.app.mouseStateService.isMiddleButtonPressed()) {
  ...
  ns.DrawingController.prototype.onMousewheel_ = function (evt) {
    ...
    var delta;
    if (pxtr.utils.UserAgent.isIE11) {
      delta = evt.wheelDelta;
    } else if (pxtr.utils.UserAgent.isFirefox) {
      delta = -40 * evt.deltaY;
    } else {
    ...
    if (pxtr.utils.UserAgent.isMac ? evt.metaKey : evt.ctrlKey) {
      modifier = modifier * 5;
  ...
  ns.DrawingController.prototype.onMouseup_ = function (event) {
    ...
    var isMiddleButton = pxtr.app.mouseStateService.isMiddleButtonPressed();
    ...
    if (this.isPickingColor || isMiddleClick) {
      ...
      this.currentToolBehavior.releaseToolAt(
        coords.x,
        coords.y,
        this.pixetorController.getCurrentFrame(),
        this.overlayFrame,
        event
      );
  ...
  ns.DrawingController.prototype.pickColorAt_ = function (coords) {
    ...
    var color = pxtr.utils.intToColor(frame.getPixel(coords.x, coords.y));
    var isRightButton = pxtr.app.mouseStateService.isRightButtonPressed();
  ...
  ns.DrawingController.prototype.render = function () {
    ...
    if (!currentFrame.isSameSize(this.overlayFrame)) {
      this.overlayFrame = pxtr.model.Frame.createEmptyFromFrame(currentFrame);
    }

    if (pxtr.UserSettings.get(pxtr.UserSettings.ONION_SKIN)) {
  ...
  ns.DrawingController.prototype.calculateZoom_ = function() {
    var frameHeight = this.pixetorController.getCurrentFrame().getHeight();
    var frameWidth = this.pixetorController.getCurrentFrame().getWidth();
