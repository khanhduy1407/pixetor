(function () {
  ...
  ns.ExportController = function (pixetorController) {
    this.pixetorController = pixetorController;
    this.tabsWidget = new pxtr.widgets.Tabs(tabs, this, pxtr.UserSettings.EXPORT_TAB);
    this.onSizeInputChange_ = this.onSizeInputChange_.bind(this);
  };

  pxtr.utils.inherit(ns.ExportController, pxtr.controller.settings.AbstractSettingController);
  ...
  ns.ExportController.prototype.init = function () {
    ...
    var scale = pxtr.UserSettings.get(pxtr.UserSettings.EXPORT_SCALE);
    this.sizeInputWidget = new pxtr.widgets.SizeInput({
      widthInput : this.widthInput,
      heightInput : this.heightInput,
      initWidth : this.pixetorController.getWidth() * scale,
      initHeight : this.pixetorController.getHeight() * scale,
  ...
  ns.ExportController.prototype.onScaleChange_ = function () {
    ...
    if (!isNaN(value)) {
      if (Math.round(this.getExportZoom()) != value) {
        this.sizeInputWidget.setWidth(this.pixetorController.getWidth() * value);
      }
      pxtr.UserSettings.set(pxtr.UserSettings.EXPORT_SCALE, value);
    }
  };
  ...
  ns.ExportController.prototype.getExportZoom = function () {
    return parseInt(this.widthInput.value, 10) / this.pixetorController.getWidth();
  };
})();
