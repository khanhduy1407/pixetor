(function () {
  ...
  ns.GifExportController = function (pixetorController, exportController) {
    this.pixetorController = pixetorController;
    this.exportController = exportController;
  };

  pxtr.utils.inherit(ns.GifExportController, pxtr.controller.settings.AbstractSettingController);
  ...
  ns.GifExportController.prototype.downloadImageData_ = function (imageData) {
    var fileName = this.pixetorController.getPixetor().getDescriptor().name + '.gif';
    pxtr.utils.BlobUtils.dataToBlob(imageData, 'image/gif', function(blob) {
  ...
  ns.GifExportController.prototype.uploadImageData_ = function (imageData) {
    ...
    pxtr.app.imageUploadService.upload(imageData,
  ...
  ns.GifExportController.prototype.renderAsImageDataAnimatedGIF = function(zoom, fps, cb) {
    ...
    var background = pxtr.utils.CanvasUtils.createCanvas(width, height);
    ...
    for (var i = 0 ; i < this.pixetorController.getFrameCount() ; i++) {
      ...
      var canvas = pxtr.utils.ImageResizer.scale(background, zoom);
    ...
    gif.on('finished', function(blob) {
      ...
      pxtr.utils.FileUtils.readFile(blob, cb);
    }.bind(this));
  ...
  ns.GifExportController.prototype.getTransparentColor = function(currentColors) {
    var transparentColor = pxtr.utils.ColorUtils.getUnusedColor(currentColors);
  ...
  ns.GifExportController.prototype.onRepeatCheckboxChange_ = function () {
    ...
    pxtr.UserSettings.set(pxtr.UserSettings.EXPORT_GIF_REPEAT, checked);
  };

  ns.GifExportController.prototype.getRepeatSetting_ = function () {
    return pxtr.UserSettings.get(pxtr.UserSettings.EXPORT_GIF_REPEAT);
  };
  ...
  ns.GifExportController.prototype.updateStatus_ = function (imageUrl, error) {
    if (imageUrl) {
      ...
      var linkHtml = pxtr.utils.Template.replace(linkTpl, {
        link : imageUrl,
