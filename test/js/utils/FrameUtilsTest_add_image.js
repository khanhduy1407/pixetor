describe("FrameUtils addImageToFrame tests", function() {
  ...
  var createFrameFromGrid = function (grid) {
    return pxtr.model.Frame.fromPixelGrid(toFrameGrid(grid));
  };

  var createImageFromGrid = function (grid) {
    return pxtr.utils.FrameUtils.toImage(createFrameFromGrid(grid));
  };
  ...
  it("adds smaller image at drop position", function () {
    ...
    pxtr.utils.FrameUtils.addImageToFrame(frame, image, 0, 0);
    ...
    pxtr.utils.FrameUtils.addImageToFrame(frame, image, 1, 1);
    // Verify
    frameEqualsGrid(frame, [
      [R, T],
      [T, R]
  ...
  it("adds line image at drop position", function () {
    ...
    pxtr.utils.FrameUtils.addImageToFrame(frame, image, 0, 1);
  ...
  it("offsets drop position", function () {
    ...
    pxtr.utils.FrameUtils.addImageToFrame(frame, image, 1, 0);
