describe("PixelUtils test suite", function() {
  ...
  it("calculates line pixels", function() {
    ...
    var pixels = pxtr.PixelUtils.getLinePixels(0, 0, 0, 0);
    expect(pixels.length).toBe(1);
    checkPixel(pixels[0], 0, 0);

    // 2-points line
    console.log('Check getLinePixels(0, 1, 0, 1)');
    pixels = pxtr.PixelUtils.getLinePixels(0, 1, 0, 1);
    ...
    pixels = pxtr.PixelUtils.getLinePixels(1, 0, 1, 0);
    expect(pixels.length).toBe(2);
    ...
    pixels = pxtr.PixelUtils.getLinePixels(0, 2, 0, 7);
  ...
  it("calculates uniform line pixels", function() {
    ...
    var pixels = pxtr.PixelUtils.getUniformLinePixels(0, 0, 0, 0);
    expect(pixels.length).toBe(1);
    checkPixel(pixels[0], 0, 0);

    // 2-points line
    console.log('Check getUniformLinePixels(0, 1, 0, 1)');
    pixels = pxtr.PixelUtils.getUniformLinePixels(0, 1, 0, 1);
    expect(pixels.length).toBe(2);
    checkPixel(pixels[0], 0, 0);
    checkPixel(pixels[1], 1, 1);
    ...
    pixels = pxtr.PixelUtils.getUniformLinePixels(1, 0, 1, 0);
    expect(pixels.length).toBe(2);
    // check pixels are returned in reverse order
    checkPixel(pixels[0], 1, 1);
    checkPixel(pixels[1], 0, 0);
    ...
    pixels = pxtr.PixelUtils.getUniformLinePixels(0, 5, 0, 2);
    ...
    pixels = pxtr.PixelUtils.getUniformLinePixels(0, 5, 0, 1);
    ...
    pixels = pxtr.PixelUtils.getUniformLinePixels(0, 2, 0, 7);
    expect(pixels.length).toBe(8);
