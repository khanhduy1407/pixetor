describe("PixelUtils visitor methods tests", function() {
  ...
  it("getSimilarConnectedPixelsFromFrame works", function() {
    ...
    var pixels = pxtr.PixelUtils.getSimilarConnectedPixelsFromFrame(frame, 0, 0);
    ...
    pixels = pxtr.PixelUtils.getSimilarConnectedPixelsFromFrame(frame, -1, -1);
    expect(Array.isArray(pixels)).toBe(true);
    expect(pixels.length).toBe(0);

    pixels = pxtr.PixelUtils.getSimilarConnectedPixelsFromFrame(frame, 0, 1);
    expect(pixels.length).toBe(1);
    expect(containsPixel(pixels, 0, 1)).toBe(true);

    pixels = pxtr.PixelUtils.getSimilarConnectedPixelsFromFrame(frame, 2, 1);
