describe("Framesheet Renderer test", function() {
  ...
  var toFrame = function (array) {
    return pxtr.model.Frame.fromPixelGrid(toFrameGrid(array));
  };
  ...
  it("draws frames side by side by default", function() {
    ...
    var renderer = new pxtr.rendering.FramesheetRenderer([f1, f2]);
    var canvas = renderer.renderAsCanvas();
    frameEqualsGrid(pxtr.utils.FrameUtils.createFromImage(canvas), [
      [B, T, W, B],
      [B, W, T, B]
    ]);
  });
  ...
  it("renderAsCanvas accepts columns argument", function() {
    ...
    // columns = 3
    renderer = new pxtr.rendering.FramesheetRenderer(frames);
