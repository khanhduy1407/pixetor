describe("Onion Skin Renderer test", function() {
  ...
  it("renders correctly :)", function() {
    ...
    var pixetorController = createMockPixetorController();
    pixetorController.currentLayer_ = layer;

    var onionSkinRenderer = new pxtr.rendering.OnionSkinRenderer(fakeRenderer, pixetorController);
    // create frame
    var previousFrame = pxtr.model.Frame.fromPixelGrid(test.testutils.toFrameGrid([
      [BLACK, TRANS],
      [TRANS, TRANS]
    ]));

    var nextFrame = pxtr.model.Frame.fromPixelGrid(test.testutils.toFrameGrid([
      [TRANS, TRANS],
      [TRANS, BLACK]
    ]));

    pixetorController.currentFrameIndex_ = 1;
    ...
    layer.frames = [{}];
    pixetorController.currentFrameIndex_ = 0;
    onionSkinRenderer.render();
  ...
  var createMockPixetorController = function () {
    return {
