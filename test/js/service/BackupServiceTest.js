describe('BackupService test', function () {
  ...
  beforeEach(function () {
    ...
    mockPixetorController = {
      getPixetor: function () { return mockPixetor; },
      setPixetor: function () {}
    };

    spyOn(pxtr.utils.serialization.Serializer, 'serialize').and.callFake(function () {
      return stubValues.serializedPixetor;
    });

    // Create test backup service with mocks.
    backupService = new pxtr.service.BackupService(
      mockPixetorController,
      mockBackupDatabase
    );
  ...
  var preparePixetorMocks = function (session_id, name, description, hash, serialized) {
    // Update the session id.
    pxtr.app.sessionId = session_id;

    // Update the pixetor mock.
    mockPixetor._descriptor.name = name;
    mockPixetor._descriptor.description = description;
    mockPixetor._hash = hash;
    stubValues.serializedPixetor = serialized;
  };

  it('calls create to backup', function (done) {
    preparePixetorMocks(1, 'pixetor_name', 'pixetor_desc', 'pixetor_hash', 'serialized');
    ...
    backupService.backup().then(function () {
      ...
      expect(snapshot.name).toEqual('pixetor_name');
      expect(snapshot.description).toEqual('pixetor_desc');
  ...
  it('does not call update to backup if the hash did not change', function (done) {
    ...
    var snapshot1 = createSnapshotObject(1, 'pixetor_name1', 'pixetor_desc1', date1, 'serialized1');
    preparePixetorMocks(session, 'pixetor_name1', 'pixetor_desc1', 'hash', 'serialized1');
    snapshotDate = date1;
    ...
    backupService.backup().then(function () {
      ...
      preparePixetorMocks(session, 'pixetor_name2', 'pixetor_desc2', 'hash', 'serialized2');
      snapshotDate = date2;
  ...
  it('calls update to backup if there is an existing & recent snapshot', function (done) {
    ...
    var snapshot1 = createSnapshotObject(1, 'pixetor_name1', 'pixetor_desc1', date1, 'serialized1');
    preparePixetorMocks(session, 'pixetor_name1', 'pixetor_desc1', 'pixetor_hash1', 'serialized1');
    snapshotDate = date1;
    ...
    backupService.backup().then(function () {
      ...
      preparePixetorMocks(session, 'pixetor_name2', 'pixetor_desc2', 'pixetor_hash2', 'serialized2');
      snapshotDate = date2;
      ...
      backupService.backup().then(function () {
        ...
        expect(snapshot.name).toEqual('pixetor_name2');
        expect(snapshot.description).toEqual('pixetor_desc2');
  ...
  it('deletes old snapshots if there are too many of them', function (done) {
    ...
    preparePixetorMocks(session, 'pixetor_name', 'pixetor_desc', 'pixetor_hash', 'serialized12');
    ...
    for (var i = maxPerSession - 1 ; i >= 0 ; i--) {
      mockBackupDatabase._sessions[session].push(
        createSnapshotObject(session, 'pixetor_name', 'pixetor_desc', i * 6 * ONE_MINUTE, 'serialized' + i)
      );
    }
