...
casper.test.begin('Simple Image import test', 27, function(test) {
  ...
  function onTestStart() {
    ...
    casper.waitForSelector('.palettes-list-color:nth-child(1)', onPixetorPaletteUpdated, test.timeout, 10000);
  }

  function onPixetorPaletteUpdated() {
    // Check the expected pixetor was correctly loaded.
    test.assertEquals(evalLine('pxtr.app.currentColorsService.getCurrentColors().length'), 1, 'Has 1 color');
  ...
  function onPopupClosed() {
    ...
    test.assertEquals(evalLine('pxtr.app.pixetorController.getLayers().length'), 2, 'Pixetor has 2 layers');
